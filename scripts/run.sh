...
case "$1" in
    up)
        ...
        docker exec -e PYTHONPATH=. -w /workspace camara-mock-gsma-tac python -m mock_services.gsma_tac.seed --count 1000000 --seed 42 || { echo "Lỗi seed GSMA"; exit 1; }
        ...
        docker exec -e PYTHONPATH=. -w /workspace camara-mock-itu-e164 python -m mock_services.itu_e164.seed --count 1000000 --seed 42 || { echo "Lỗi seed ITU"; exit 1; }
