        ...
        while i < n:
            ...
            if conflict_type == "A":
                ...
                try:
                    ...
                    mid = t0 + (t1 - t0) / 2
                    extra_start["event_timestamp"] = mid.strftime("%Y-%m-%dT%H:%M:%SZ")
                    extra_start["ingest_timestamp"] = (mid + timedelta(seconds=2)).strftime("%Y-%m-%dT%H:%M:%SZ")
                ...
                if t_swap_start is not None:
                    swap_start["event_timestamp"] = t_swap_start.strftime("%Y-%m-%dT%H:%M:%SZ")
                    swap_start["ingest_timestamp"] = (t_swap_start + timedelta(seconds=2)).strftime("%Y-%m-%dT%H:%M:%SZ")
                    swap_stop["event_timestamp"] = t_swap_stop.strftime("%Y-%m-%dT%H:%M:%SZ")
                    swap_stop["ingest_timestamp"] = (t_swap_stop + timedelta(seconds=2)).strftime("%Y-%m-%dT%H:%M:%SZ")
                ...
                same_msisdn = base_subscriber(cur_idx)["msisdn"]
                same_imsi = base_subscriber(cur_idx)["imsi"]
